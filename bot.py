...
def handle_exception(e: Exception, context: str = ""):
    ...
    if isinstance(e, UnicodeDecodeError):
        print(f"⚠️ Ignored UnicodeDecodeError in {context}")
        return
    if is_minor_exception(e):
        print(f"⚠️ Minor error in {context}: {e}")
    else:
        print(f"❌ Serious error in {context}: {e}")
        raise
...
def read_money_box():
    try:
        ...
        text = safe_image_to_string(thresh)
...
def check_disconnected():
    try:
        ...
        for i, word in enumerate(data.get("text", [])):
            if "disconnected" in str(word).lower():
...
def reconnect_if_disconnected():
    ...
    while True:
        try:
            ...
            handle_exception(e, "reconnect_if_disconnected")
            time.sleep(2)
...
def spin_sequence():
    def hold_extra(key):
        ...
        try:
            ...
            handle_exception(e, "spin_hold_extra")
...
def movement_loop():
    ...
    while running:
        try:
            ...
            handle_exception(e, "movement_loop")
