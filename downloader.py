...
def auto_update():
    try:
        ...
        if r.status_code != 200:
            print(f"[!] Failed to fetch remote file (status {r.status_code})")
            return
        remote_code = r.text
        ...
        if remote_code.strip() != local_code.strip():
            print("[!] Update found! Replacing local file...")
            with open(LOCAL_FILE, "w", encoding="utf-8") as f:
                f.write(remote_code)
            print("[+] Update complete. Restarting bot...")
            os.execv(sys.executable, ["python"] + sys.argv)  # Relaunch with updated file
        else:
            print("[*] Already up to date.")
    except Exception as e:
        print("[!] Auto-update failed:", e)

# Run updater before anything else
auto_update()
